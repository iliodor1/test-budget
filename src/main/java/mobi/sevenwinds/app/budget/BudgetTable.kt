...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponseStats(): BudgetStatsRecord {
        val formatter = DateTimeFormat.forPattern("dd.MM.yyyy HH:mm:ss")
        val createdDateTime = author?.createdAt?.toString(formatter)

        return BudgetStatsRecord(year, month, amount, type, author?.fullName, createdDateTime)
    }
}